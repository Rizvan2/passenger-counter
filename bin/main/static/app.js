...
async function startSession() {
  ...
  try {
    ...
    void loadVideoInfo(selectedPath);
...
// ═══════════════════════════════════════════════════
// MONITOR — WebSocket, буфер кадров и таймлайн
// ═══════════════════════════════════════════════════
...
function positionOfFrame(frameIndex) {
  const idx = frameTimeline.indexOf(frameIndex);
  return idx >= 0 ? idx : Math.max(0, frameTimeline.length - 1);
}
...
function updateTimelineMeta() {
  ...
  const totalEstimate = videoInfo?.lengthInFrames
    ? Math.max(videoInfo.lengthInFrames, frameTimeline[frameTimeline.length - 1] + 1)
    : (processedFrames || frameTimeline[frameTimeline.length - 1] + 1);
...
function formatClock(frameIndex) {
  const fps = videoInfo?.frameRate > 0 ? videoInfo.frameRate : 25;
...
async function loadVideoInfo(path) {
  videoInfo = null;
  try {
    const r = await fetch(`/api/videos/info?path=${encodeURIComponent(path)}`);
    if (r.ok) videoInfo = await r.json();
  } catch (e) {
    // метаданные опциональны — таймлайн работает и без них
  }
  updateTimelineMeta();
}
...
async function renderFrameMessage(msg, appendEvents) {
  ...
  try {
    ...
    if (token === displayToken) setStatus("ошибка кадра", "error");
  }
}

function applyFrameStats(msg) {
